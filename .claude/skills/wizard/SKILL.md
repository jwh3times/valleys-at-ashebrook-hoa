---
# GENERATED — do not edit. Source: .agents/skills/wizard/SKILL.md
name: wizard
...
A wizard is ephemeral by default — built for one run, saved to a scratch path, deleted when the job's done. In this repo that scratch path is `private/` (gitignored) — **not** `scripts/`, which holds committed project automation. Move it into `scripts/` only when the user wants a repeatable setup path that should live in the repo.
