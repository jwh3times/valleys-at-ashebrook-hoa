...
<!-- Generated by `npm run agents:sync` from .claude/agents/code-reviewer.md. Edit that file, not this one. -->

> **Delegated role.** These are the instructions for the `code-reviewer` role,
> mirrored from the Claude Code subagent at `.claude/agents/code-reviewer.md`. Run them in a
