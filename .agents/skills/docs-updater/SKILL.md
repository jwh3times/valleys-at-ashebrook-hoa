...
<!-- Generated by `npm run agents:sync` from .claude/agents/docs-updater.md. Edit that file, not this one. -->

> **Delegated role.** These are the instructions for the `docs-updater` role,
> mirrored from the Claude Code subagent at `.claude/agents/docs-updater.md`. Run them in a
