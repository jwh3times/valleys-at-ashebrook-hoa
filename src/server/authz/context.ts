...
export async function getAuthContext(request: Request, env: Env): Promise<AuthContext | null> {
  ...
  const [roleRows, links] = await Promise.all([
    db.select({ role: users.role }).from(users).where(eq(users.id, result.user.id)),
    db.select({ ownerId: userPropertyLinks.ownerId }).from(userPropertyLinks).where(eq(userPropertyLinks.userId, result.user.id)),
  ]);
  const rawRole = roleRows[0]?.role ?? null;
  const role: Role = rawRole && VALID_ROLES.has(rawRole) ? (rawRole as Role) : 'visitor';
