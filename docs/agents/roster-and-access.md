...
capabilities) instead of the session's `role` mirror. What remains of wave 1 — the
`role`/`propertyIds` compatibility aliases and the `user_property_links` readers (`content/voting.ts`,
