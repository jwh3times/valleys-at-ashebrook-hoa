...
describe('firstEmail', () => {
  it('takes the first address from a multi-email cell', () => {
    expect(firstEmail('[removed], [removed]')).toBe(
      '[removed]',
    );
  });
...
describe('rowsToRoster', () => {
  it('maps a row to one property + its first homeowner', () => {
    const { properties, owners } = rowsToRoster([
      {
        'Homeowner 1': 'Homeowner A',
        'Homeowner 1 Phone': '(919) 451-7647',
        'Homeowner 1 Email': '[removed]',
    ...
    expect(owners[0].fullName).toBe('Homeowner A');
    expect(owners[0].phone).toBe('+19194517647');
    expect(owners[0].email).toBe('[removed]');
  });
  ...
  it('emits a second owner when Homeowner 2 is present, sharing the propertyId', () => {
    const { owners } = rowsToRoster([
      {
        ...
        'Homeowner 2': 'Homeowner B',
        'Homeowner 2 Phone': '(630) 707-1482',
        'Homeowner 2 Email': '[removed]',
    ...
    expect(owners[1].email).toBe('[removed]');
  });
